    ...
    companion object {
        ...
        fun retrieveByPriceDrop(url: String, percent: Boolean, drop: Int): List<String> {
            ...
            for (elem in elements) {
                ...
                if (element.childNode(1).childNodeSize() == 6) {
                    try {
                        val priceDrop = element.childNode(1).childNode(4).childNode(1).childNode(0).toString().replace("Price dropped ", "").replace("%", "").trim().toInt()
